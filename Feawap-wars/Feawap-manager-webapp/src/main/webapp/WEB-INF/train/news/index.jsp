	...
	<c:choose>
		<c:when test="${empty list}">
			<div class="alert alert-info">
				...
				还没有添加任何培训动态！请点击
				<a href="<%=path %>/action/train/news/record" class="btn btn-xs btn-primary"><i class="icon-plus"></i> 添加</a>
				新增培训动态
			</div>
		</c:when>
		<c:otherwise>
							...
							<tr>
								...
								<td>
									<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
										<a href="<%=path %>/action/train/news/record?id=${record.id}" class="btn btn-xs btn-info">
									...
									<div class="visible-xs visible-sm hidden-md hidden-lg">
										<div class="inline position-relative">
											...
											<ul class="dropdown-menu dropdown-only-icon dropdown-yellow pull-right dropdown-caret dropdown-close">
												<li>
													<a href="<%=path %>/action/train/news/record?id=${record.id}" class="tooltip-success" data-rel="tooltip" title="" data-original-title="Edit">
														<span class="green">
															<i class="icon-edit bigger-120"></i>
														</span>
													</a>
				...
				buttons: [
					{
						...
						click: function() {
							window.location.href = "<%=path %>/action/train/news/delete?id=" + id;
						}
					}
					,
					{
