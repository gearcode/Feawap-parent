...
<html lang="en">
	...
	<body>
		...
		<div class="confirm-bar">
			<p>
				<c:choose>
					<c:when test="${url==null&&go==null }">
						<button type="button" onclick="history.back()">确定</button>
					</c:when>
					<c:when test="${url==null&&go!=null }">
						<button type="button" onclick="history.go(${go})">确定</button>
					</c:when>
					<c:when test='${fn:indexOf(url,"http://") ==0 }'>
						<button type="button" onclick="location.href='${url}'">确定</button>
					</c:when>
					<c:otherwise>
